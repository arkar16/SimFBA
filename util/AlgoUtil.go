...
func GetHeismanScore(cp structs.CollegePlayer, weightMap map[string]float64, homeTeamMapper map[int]string) float64 {
	...
	for _, stat := range stats {
		...
		statScore -= (float64(stat.Interceptions) * 2.125)
		statScore -= (float64(stat.Sacks) * 2.125)
		...
		statScore += (float64(stat.TacklesForLoss) * 6.5)
		statScore += (float64(stat.SacksMade) * 8)
		statScore += (float64(stat.PassDeflections) * 6.5)

		statScore += (float64(stat.ForcedFumbles) * 10)
		statScore += (float64(stat.RecoveredFumbles) * 6.75)
		statScore += (float64(stat.InterceptionsCaught) * 15)
		statScore += (float64(stat.PuntsBlocked) * 10)

		statScore += (float64(stat.Safeties) * 12)
		statScore += (float64(stat.DefensiveTDs) * 30)
		statScore += (float64(stat.KickReturnTDs) * 15)
		statScore += (float64(stat.PuntReturnTDs) * 15)
		statScore += (float64(stat.FGBlocked) * 15)
